...
cp v13/reset-v138.js "$WWW/reset-v138.js"
node --check "$WWW/theme-v138.js"
node --check "$WWW/reset-v138.js"
...
for script in ['/theme-v138.js','/reset-v138.js']:
    if script not in s:
        s=s.replace('</body>',f'<script src="{script}"></script></body>')
home.write_text(s)
for rel in ['copa/index.html','eliminatorias/index.html','mercado/index.html','galeria/index.html']:
    p=root/rel
    s=p.read_text(errors='ignore')
    s=s.replace('<script src="/theme-v138.js"></script>','').replace('<script src="/reset-v138.js"></script>','')
    inject='<script src="/theme-v138.js"></script><script src="/reset-v138.js"></script>'
...
reset=(root/'reset-v138.js').read_text(errors='ignore')
...
for rel in ['index.html','copa/index.html','eliminatorias/index.html','mercado/index.html','galeria/index.html']:
    ...
    assert '/reset-v138.js' in text, f'Reset v138 não injetado em {rel}'
...
assert 'Zerar Copa' in reset and 'ÚLTIMA CONFIRMAÇÃO' in reset
assert 'CopaProfiles.backup()' in reset
assert "'copaOuroV3'" in reset and "'copaOuroTeamDraft'" in reset and "'copaOuroGameRouletteV135'" in reset
assert "removeItem('elimCopaOuro')" not in reset
assert "removeItem('copaOuroQualified')" not in reset
assert "removeItem('copaOuroMercadoV2')" not in reset
assert "removeItem('copaOuroHistory')" not in reset
assert "removeItem('copa-ouro-edicoes-v1')" not in reset
...
for f in ['profile-manager-v137.js','cloud-sync-v137.js','navigation-v135.js','edition-automation-v136.js','stability-v137.js','market-sync.js','crest-auto.js','team-name-helper.js','theme-v138.js','reset-v138.js']:
    p=root/f
    assert p.exists() and p.stat().st_size>0, f'Arquivo ausente: {f}'
print('PENTE_FINO_VISUAL_RESET_V138_OK')
PY

for f in profile-manager-v137.js cloud-sync-v137.js navigation-v135.js edition-automation-v136.js stability-v137.js market-sync.js crest-auto.js team-name-helper.js theme-v138.js reset-v138.js; do
  node --check "$WWW/$f"
done
...
for f in \
  assets/www/theme-v138.js assets/www/reset-v138.js \
...
node --check /tmp/v138check/assets/www/reset-v138.js
...
grep -q 'Zerar Copa' /tmp/v138check/assets/www/reset-v138.js
grep -q 'ÚLTIMA CONFIRMAÇÃO' /tmp/v138check/assets/www/reset-v138.js
grep -q 'CopaProfiles.backup()' /tmp/v138check/assets/www/reset-v138.js
