...
DIST="$ROOT/dist"
rm -rf app "$DIST" aligned-v052.apk release-v052.jks

cat > settings.gradle <<'GRADLE'
pluginManagement { repositories { google(); mavenCentral(); gradlePluginPortal() } }
dependencyResolutionManagement { repositoriesMode.set(RepositoriesMode.FAIL_ON_PROJECT_REPOS); repositories { google(); mavenCentral() } }
rootProject.name='RumiMixArenaV052Compat'
include ':app'
GRADLE
...
mkdir -p app/src/main/java/com/rumix/arena/compat app/src/main/res/values app/src/main/res/drawable "$WWW" "$DIST"
sed 's/package com\.rumix\.arena\.online;/package com.rumix.arena.compat;/' rumix-v043/MainActivity.java > app/src/main/java/com/rumix/arena/compat/MainActivity.java
...
android {
  namespace 'com.rumix.arena.compat'
  compileSdk 35
  defaultConfig {
    applicationId 'com.rumix.arena.compat.v052'
    minSdk 21
    targetSdk 35
    versionCode 52
    versionName '0.5.2'
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
  ...
  <application android:theme="@style/AppTheme" android:label="Rumi Mix Arena" android:icon="@drawable/app_icon">
    <activity android:name="com.rumix.arena.compat.MainActivity" android:exported="true" android:screenOrientation="landscape">
      <intent-filter>
...
"$BTDIR/zipalign" -p -f 4 "$UNSIGNED" aligned-v052.apk

PASS="RMA052$(date +%s)$(openssl rand -hex 6)"
keytool -genkeypair -noprompt -keystore release-v052.jks -storepass "$PASS" -keypass "$PASS" -alias rumixarena -keyalg RSA -keysize 2048 -validity 10000 -dname 'CN=Rumi Mix Arena, OU=Android, O=Rumi Mix Arena, L=Sao Paulo, ST=SP, C=BR'

APK="$DIST/RumiMixArena-v0.5.2-COMPAT.apk"
"$BTDIR/apksigner" sign --ks release-v052.jks --ks-key-alias rumixarena --ks-pass "pass:$PASS" --key-pass "pass:$PASS" --v1-signing-enabled true --v2-signing-enabled true --v3-signing-enabled false --out "$APK" aligned-v052.apk
...
grep -q "package: name='com.rumix.arena.compat.v052'" "$DIST/APK-INFO.txt"
...
Rumi Mix Arena v0.5.2 COMPAT

Versão criada especificamente para reduzir falhas de instalação:
- pacote Android totalmente novo: com.rumix.arena.compat.v052
...
- sem deep link
- sem roundIcon
- sem atributos opcionais extras
- Online mantido
- CPU, Treino e MegaRumiX mantidos
- visual e gestos da v0.5 mantidos
- única permissão: INTERNET
TXT

(cd "$DIST" && zip -9 RumiMixArena-v0.5.2-COMPAT.zip RumiMixArena-v0.5.2-COMPAT.apk SHA256.txt ASSINATURA.txt APK-INFO.txt PERMISSOES.txt LEIA-ME.txt)
unzip -t "$DIST/RumiMixArena-v0.5.2-COMPAT.zip"
echo 'RUMIX_V052_COMPAT_OK'
